...
if (isset($_GET['prueba']) && $_GET['prueba'] == 1) {
    $host_url = "http://192.168.220.28/";
    $url = "http://192.168.220.28/revistas";
} else {
    $host_url = "http://www.icesi.edu.co/";
...
switch ($_GET['revista']) {
    case 0:
        $path = "n00b";
        $_GET['biblio_apps'] = $host_url . "se/le/olvido/la/revista";
        break;
    case 1:
        $path = "estudios_gerenciales";
        $_GET['biblio_apps'] = $host_url . "estudios_gerenciales/buscador";
        break;
    case 2:
        $path = "revista_cs";
        $_GET['biblio_apps'] = $host_url . "/revista_cs/buscador";
        break;
    case 4:
        $path = "sistemas_telematica";
        $_GET['biblio_apps'] = $host_url . "sistemas_telematica/buscador";
        break;
}
...
/**
 * This part is commented because OJS nos expects searchType parameter to be a string
 */
//if (!is_numeric($searchType))
//  $searchType = null;
/**
 * damanzano
 * In OJS 2.4.5 the method ArticleSearch::retrieveResults parse the query before trying to get the results,
 * so parsing the query before calling this metohd result in an error generated by doble parsing.
 * 
 * Instead, you need to get the keywords corresponding to the search parameters. 
 */
//$keywords = array($searchType => ArticleSearch::_parseQuery($_GET['query']));
$searchFilters = array(
    'query' => $_GET['query'],
...
$searchFilters [$searchType]= $_GET['query'];
//echo "My filters: ";
...
$error='';
$resultadosPKP =& ArticleSearch::retrieveResults($journal, $keywords, $error, null, null, $rangeInfo);

foreach ($resultadosPKP as $resultadoPKP) {
    $detalles[] = $resultadoPKP;
}
/**
